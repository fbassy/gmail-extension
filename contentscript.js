var UNL_DEFAULT_HOST_NAME = "api.unleashedsoftware.com";
...
function bindEventToGmail() {
  ...
  chrome.storage.local.get("unl_key", function (fetchedData) { // Get key config
    var keyValue = fetchedData.unl_key;
    if (!keyValue) {
      // keyValue = UNL_DEFAULT_HOST_NAME;
    }

    bindEmailMouseOverEvent(keyValue);
 ...
 */
function bindEmailMouseOverEvent(unlHostName) {
  var xhr;
  $("span[email]").unbind('mouseenter').unbind('mouseleave').hover(function () {
    ...
    var url = "https://api.unleashedsoftware.com/Customers?Email=test@test.co.nz";
        ...
        xhr =
          $.ajax({
            ...
            headers: {
              ...
              'api-auth-id':'aad7f3fd-87fa-465a-9a5b-7b0efd997afd',
